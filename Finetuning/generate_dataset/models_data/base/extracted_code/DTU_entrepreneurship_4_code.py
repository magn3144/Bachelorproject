...
The task is: For each course on the page get the semester and course name of that course. The course titles start with a 5-digit number.
...
for course in soup.find_all('h2'):
    course_name = course.text.strip()
    course_names.append(course_name)

for course in soup.find_all('span'):
    course_semester = course.text.strip()
    course_semesters.append(course_semester)

course_data = pd.DataFrame({'Course Name': course_names, 'Semester': course_semesters})
course_data.to_csv('scraped_data.csv', index=False)
