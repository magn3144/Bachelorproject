...
def reload_model(model_name, dataset, r, epochs):
...
save_directory = "/work3/s204164/LLAMA2_Finetuning/trained_models"
dataset = load_dataset(dataset_name, split="train[0:860]") #860
print("2a: dataset loaded")

r_values = [1, 2, 4, 8, 16, 32][::-1]
epochs_values = [1, 2, 4, 8, 16, 32]
print("r values:", r_values)
print("epochs range:", epochs_values)
for r in r_values:
    for epochs in epochs_values:
        finetune_name = f"finetuned_r{r}_e{epochs}"
        print(f"finetune_name: {finetune_name}")
        trainer, model = reload_model(model_name, dataset, r, epochs)
        trainer.train()
